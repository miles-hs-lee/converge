    ...
    <>
      ...
      <div className="mt-3 space-y-2 text-sm">
        {sortMode === "default"
          ? filtered.map((person) => (
              <button
                className="block w-full rounded-xl border border-line bg-white/90 px-3 py-3 text-left transition hover:border-accent/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/35"
                key={person.id}
              ...
              <section className="rounded-xl border border-line bg-white/75 p-2" key={group.tenantName}>
                ...
                <div className="space-y-2">
                  {group.items.map((person) => (
                    <button
                      className="block w-full rounded-xl border border-line bg-white px-3 py-3 text-left transition hover:border-accent/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/35"
                      key={person.id}
                      onClick={() => setSelectedPersonId(person.id)}
                      type="button"
                    >
      ...
      {selectedPerson && actionLinks ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4" role="dialog" aria-modal="true">
          ...
          <section className="panel-glass card relative z-10 w-full max-w-2xl p-5">
